...
vvas_xinitmodel (vvas_xkpriv * kpriv, int modelclass)
{
  ...
  switch (modelclass) {
    ...
    {
      model = new vvas_xrefinedet (kpriv, kpriv->elfname, kpriv->need_preprocess);
      break;
    }
    
    case VVAS_XCLASS_MEDICALREFINEDET:
    {
      model =  new vvas_xmedicaldet (kpriv, kpriv->elfname, kpriv->need_preprocess);
      break;
    }
#endif
#ifdef ENABLE_TFSSD
